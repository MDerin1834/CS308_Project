...
function generateInvoicePdf({ order, user }) {
  return new Promise((resolve, reject) => {
    ...
    const chunks = [];
    const invoiceNumber = order.invoiceNumber || buildInvoiceNumber(order.id || order._id);
    ...
    doc.text(`Customer: ${user?.username || order.shippingAddress?.fullName || "Customer"}`);
    doc.moveDown();
    ...
    order.items?.forEach((item) => {
      ...
      doc.text(
        `Quantity: ${item.quantity}  Unit: ${formatCurrency(item.unitPrice)}  Line: ${formatCurrency(
          item.lineTotal
        )}`
      );
      doc.moveDown(0.4);
